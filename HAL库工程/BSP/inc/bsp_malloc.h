...
# define USE_SRAMCCM	1u   /* 是否使用内部CCM内存，对F1无效 */
...
# if USE_SRAMIN > 0u
	...
	# define SRAMIN_MAX_SIZE		64*1024							/*  最大内存96  */	
...
//extern Malloc_Str malloc_dev;				/*    */

void bsp_mem_Set(void *s, uint8_t value, uint32_t len);		/*  设置内存  */
void bsp_mem_Copy(void *des, void *src, uint32_t len);			/*  复制内存  */
void bsp_mem_Config(void);																		/*  内存管理初始化  */

//uint32_t bsp_mem_mallocIN(uint8_t memx, uint32_t size);				/*  内存分配，内部调用  */
//uint8_t bsp_mem_freeIN(uint8_t memx, uint32_t offset);					/*  内存释放，内部调用  */

uint8_t bsp_mem_GetUsed(uint8_t memx);												/*  获取内存使用率  */

void bsp_mem_Free(uint8_t memx, void *mem);										/*  内存申请  */
void *bsp_mem_Malloc(uint8_t memx, uint32_t size);							/*  内存释放  */
