...
SD_Error bsp_sd_PowerCmd(FunctionalState cmd)
{
	...
	if(cmd == ENABLE)
	{
		...
		bsp_sdio_SetClk(SDIO_INIT_CLK_DIV);
		SDIO->POWER = 0x03;
		SDIO->CLKCR |= 1 << 8;
		...
		sd_err = bsp_sd_CmdR1Error(SD_CMD_APP_CMD);
		if(sd_err == SD_OK)
		{
...
SD_Error bsp_sd_CardInit(void)
{
	   ...
	   (SDIO_HIGH_CAPACITY_SD_CARD==CardType))
	{
		...
		sd_err = bsp_sd_CmdR6Error(SD_CMD_SET_REL_ADDR, &rca);
		if(sd_err != SD_OK) return sd_err;
	} /*  end of if 卡类型判断  */
...
SD_Error bsp_sd_Select(uint32_t addr)
{
	...
	return bsp_sd_CmdR1Error(SD_CMD_SEL_DESEL_CARD);
}
...
SD_Error bsp_sd_SetStatus(uint32_t *CardStatus)
{
	...
	errorstatus=bsp_sd_CmdR1Error(SD_CMD_SEND_STATUS);	//查询响应状态 
...
SD_Error bsp_sd_ReadBlock(uint8_t *buff, long long addr, uint16_t blksize)
{
	...
	if((blksize>0)&&(blksize<=2048)&&((blksize&(blksize-1))==0))
	{
		...
		errorstatus=bsp_sd_CmdR1Error(SD_CMD_SET_BLOCKLEN);	//等待R1响应   
	...
	errorstatus=bsp_sd_CmdR1Error(SD_CMD_READ_SINGLE_BLOCK);//等待R1响应   
...
SD_Error bsp_sd_ReadBlocks(uint8_t *buff, long long addr, uint16_t blksize, uint32_t nblks)
{
  ...
  bsp_sdio_SendData_Cfg(SD_DATATIMEOUT,0,0,0);	//清除DPSM状态机配置
	...
	{									    
		...
		errorstatus=bsp_sd_CmdR1Error(SD_CMD_READ_MULT_BLOCK);//等待R1响应   
		if(errorstatus!=SD_OK)return errorstatus;   	//响应错误	  
 		if(DeviceMode==SD_POLLING_MODE)
		{
	 		...
	 		if(SDIO->STA&(1<<8))		//接收结束
			{
				if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
				{
					...
					errorstatus=bsp_sd_CmdR1Error(SD_CMD_STOP_TRANSMISSION);//等待R1响应   
					if(errorstatus!=SD_OK)return errorstatus;	 
				}
 			}
			ENABLE_INT();//开启总中断
...
SD_Error bsp_sd_WriteBlock(uint8_t *buff, long long addr, uint16_t blksize)
{
  ...
  bsp_sdio_SendCmd(SD_CMD_SEND_STATUS,1,(uint32_t)RCA<<16);	//发送CMD13,查询卡的状态,短响应 	   
	errorstatus=bsp_sd_CmdR1Error(SD_CMD_SEND_STATUS);		//等待R1响应   		   
	if(errorstatus!=SD_OK)return errorstatus;
	...
	errorstatus=bsp_sd_CmdR1Error(SD_CMD_WRITE_SINGLE_BLOCK);//等待R1响应   		   
	if(errorstatus!=SD_OK)return errorstatus;   	  
...
SD_Error bsp_sd_WriteBlocks(uint8_t *buff, long long addr, uint16_t blksize, uint32_t nblks)
{
	...
	{					  
     	...
     	if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||(SDIO_STD_CAPACITY_SD_CARD_V2_0==CardType)||(SDIO_HIGH_CAPACITY_SD_CARD==CardType))
    	{
			...
			errorstatus=bsp_sd_CmdR1Error(SD_CMD_APP_CMD);		//等待R1响应   		   
			if(errorstatus!=SD_OK)return errorstatus;				    
	 	   	bsp_sdio_SendCmd(SD_CMD_SET_BLOCK_COUNT,1,nblks);	//发送CMD23,设置块数量,短响应 	   
			errorstatus=bsp_sd_CmdR1Error(SD_CMD_SET_BLOCK_COUNT);//等待R1响应   		   
			if(errorstatus!=SD_OK)return errorstatus;				    
		} 
		bsp_sdio_SendCmd(SD_CMD_WRITE_MULT_BLOCK,1,addr);		//发送CMD25,多块写指令,短响应 	   
		errorstatus=bsp_sd_CmdR1Error(SD_CMD_WRITE_MULT_BLOCK);	//等待R1响应   		   
		if(errorstatus!=SD_OK)return errorstatus;
...
SD_Error bsp_sd_ProcessIRQSrc(void)
{
	...
	{	 
		...
		{
			bsp_sdio_SendCmd(SD_CMD_STOP_TRANSMISSION, 1, 0);		//发送CMD12,结束传输 	   
			TransferError = bsp_sd_CmdR1Error(SD_CMD_STOP_TRANSMISSION);
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR0Error(void)
{
	SD_Error sd_err = SD_OK;
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR1Error(uint8_t cmd)
{
	uint32_t status; 
	while(1)
	{
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR2Error(void)
{
		SD_Error errorstatus=SD_OK;
	uint32_t status;
	uint32_t timeout=SDIO_CMD0TIMEOUT;
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR3Error(void)
{
		uint32_t status;						 
 	while(1)
	{
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR6Error(uint8_t cmd, uint16_t *prca)
{
	SD_Error errorstatus=SD_OK;
	uint32_t status;					    
	uint32_t rspr1;
...
*********************************************************************************************************
*/
SD_Error bsp_sd_CmdR7Error(void)
{
	SD_Error sd_err = SD_OK;
	uint32_t status;
	uint32_t timeout = SDIO_CMD0TIMEOUT;
...
SD_Error bsp_sd_EnWideBus(uint8_t en)
{
	...
	if((scr[1]&SD_WIDE_BUS_SUPPORT)!=SD_ALLZERO)		//支持宽总线
	{
	 	...
	 	errorstatus=bsp_sd_CmdR1Error(SD_CMD_APP_CMD);
	 	if(errorstatus!=SD_OK)return errorstatus; 
	 	bsp_sdio_SendCmd(SD_CMD_APP_SD_SET_BUSWIDTH,1,arg);//发送ACMD6,短响应,参数:10,4位;00,1位.											  
		errorstatus=bsp_sd_CmdR1Error(SD_CMD_APP_SD_SET_BUSWIDTH);
		return errorstatus;
...
SD_Error bsp_sd_FindSCR(uint16_t rca, uint32_t *pscr)
{
	...
	uint32_t tempscr[2]={0,0};  
 	...
 	errorstatus=bsp_sd_CmdR1Error(SD_CMD_SET_BLOCKLEN);
 	if(errorstatus!=SD_OK)return errorstatus;	    
	...
	bsp_sdio_SendCmd(SD_CMD_SD_APP_SEND_SCR,1,0);		//发送ACMD51,短响应,参数为0											  
 	errorstatus=bsp_sd_CmdR1Error(SD_CMD_SD_APP_SEND_SCR);
 	if(errorstatus!=SD_OK)return errorstatus;							 
